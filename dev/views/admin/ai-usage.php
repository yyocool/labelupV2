...
<div class="admin-head">
  <div>
    <h1>사용량 통계</h1>
    <p>홈·편집기 라비AI 요청 수, 토큰, 예상 금액(원)을 기간별로 확인합니다.</p>
  </div>
  <div class="admin-head-actions">
    <a class="admin-btn" href="<?= url('admin/ai/token-logs') ?>">토큰사용로그</a>
    <a class="admin-btn" href="<?= url('admin/ai/member-usage') ?>">회원별 사용</a>
  <form class="admin-toolbar admin-toolbar--inline" method="get" action="<?= url('admin/ai/usage') ?>">
...
<div class="admin-kpis">
  ...
  <div class="admin-kpi"><div class="lbl">예상금액(원)</div><div class="val"><?= e(format_ai_krw($s['cost_krw'] ?? 0)) ?></div></div>
</div>
<div class="admin-kpis admin-kpis--sub">
  ...
  <div class="admin-kpi admin-kpi--sm"><div class="lbl">이용 회원</div><div class="val"><?= number_format((int) ($s['users'] ?? 0)) ?></div></div>
</div>

<div class="admin-card admin-dash-menus-wrap">
  <h2>일별 요청</h2>
  <div class="admin-table-wrap">
    <table class="admin-table">
      <thead>
        <tr><th>날짜</th><th>요청</th><th>성공</th><th>실패</th><th>토큰</th><th>예상금액</th></tr>
      </thead>
      <tbody>
      <?php if (empty($stats['daily'])): ?>
        <tr><td colspan="6" class="empty">해당 기간 사용 기록이 없습니다.</td></tr>
      <?php else: ?>
  ...
  <div class="admin-table-wrap">
    <table class="admin-table">
      ...
      <?php if (empty($stats['recent'])): ?>
        <tr><td colspan="7" class="empty">아직 사용 기록이 없습니다. 홈이나 편집기에서 라비AI를 사용하면 여기에 쌓입니다.</td></tr>
      <?php else: ?>
      <?php foreach ($stats['recent'] as $row): ?>
        <tr>
          <td><small><?= e(substr((string) ($row['created_at'] ?? ''), 0, 16)) ?></small></td>
          <td><?= e((string) ($row['member_label'] ?? $row['email'] ?? ('#' . (int) ($row['user_id'] ?? 0)))) ?></td>
          <td><?= e((string) ($row['surface_label'] ?? AiUsageService::surfaceLabel((string) ($row['surface'] ?? '')))) ?></td>
          <td><?= e((string) ($row['intent_label'] ?? AiUsageService::intentLabel((string) ($row['intent'] ?? '')))) ?></td>
