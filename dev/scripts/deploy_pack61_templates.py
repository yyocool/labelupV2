#!/usr/bin/env python3
"""Upload pack61 template assets + seed on remote."""
...
def main() -> None:
    ...
    for rel in [
        ...
        "scripts/generate_template_pack61.py",
        "scripts/lib_pack60_arts.py",
        "storage/imports/template_pack61_manifest.json",
        "storage/imports/pack61_reference.png",
    ]:
        local = LOCAL / rel
        if local.is_file():
            files.append((local, f"{REMOTE}/{rel}"))

    art_dir = LOCAL / "public" / "assets" / "templates" / "pack61"
    for p in sorted(art_dir.glob("*.png")):
        files.append((p, f"{REMOTE}/public/assets/templates/pack61/{p.name}"))
    ...
    for cmd in [
        "cd /home/labelupdev && php -l app/Services/LabelTemplatePack60SeedService.php",
        "cd /home/labelupdev && php scripts/seed_templates.php --force",
        "ls /home/labelupdev/public/assets/templates/pack61/*_art.png | wc -l",
    ]:
        print("$", cmd)
